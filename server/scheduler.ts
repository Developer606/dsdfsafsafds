...
async function processScheduledBroadcasts() {
  try {
    ...
    const broadcastsDue = scheduledBroadcasts.filter(
      (broadcast) =>
    ...
    for (const broadcast of broadcastsDue) {
      try {
        ...
        console.log(
          `Successfully sent scheduled broadcast: ${broadcast.title}`,
        );
      } catch (error) {
        console.error(
          `Error sending scheduled broadcast ${broadcast.id}:`,
          error,
        );
      }
    }
