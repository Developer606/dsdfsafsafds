...
import { adminUsers, apiKeys, type InsertAdminUser, type InsertApiKey } from '@shared/admin-schema';
...
async function initializeDefaultAdmin() {
  try {
    ...
    const existingAdmin = await adminDb.select().from(adminUsers).where(eq(adminUsers.username, 'SysRoot_99')).get();
    ...
    if (!existingAdmin) {
      ...
      console.log('Default admin user created in admin database');
    }
  } catch (error) {
    console.error('Error creating default admin user in admin database:', error);
  }
}
...
async function transferGithubToken() {
  ...
  try {
    ...
    const existingKey = await adminDb.select().from(apiKeys).where(eq(apiKeys.service, 'GITHUB_TOKEN')).get();
...
export async function getApiKey(service: string): Promise<string | null> {
  try {
    const result = await adminDb.select().from(apiKeys).where(eq(apiKeys.service, service)).get();
...
export async function setApiKey(service: string, key: string, description?: string): Promise<boolean> {
  try {
    const existingKey = await adminDb.select().from(apiKeys).where(eq(apiKeys.service, service)).get();
...
export async function verifyAdminCredentials(username: string, password: string): Promise<boolean> {
  try {
    const admin = await adminDb.select().from(adminUsers).where(eq(adminUsers.username, username)).get();
...
// Add new admin user
export async function addAdminUser(userData: InsertAdminUser): Promise<boolean> {
  try {
