...
import { initializeCharacterDb } from "./character-db";
  ...
  try {
    ...
    for (const column of columnsToAdd) {
      if (!existingColumns.has(column.name)) {
        ...
        sqlite
          .prepare(`ALTER TABLE users ADD COLUMN ${column.name} ${column.sql}`)
          .run();
      }
    }
...
export async function runMigrations() {
  ...
  try {
    ...
    createIndexes();
    console.log("Database, plans, and character database initialization completed");
  } catch (error: any) {
...
function createIndexes() {
  try {
    ...
    sqlite.transaction(() => {
      ...
      sqlite
        .prepare(
          "CREATE INDEX IF NOT EXISTS idx_messages_char_time ON messages(character_id, timestamp)",
        )
        .run();
      ...
      sqlite
        .prepare(
          "CREATE INDEX IF NOT EXISTS idx_pending_verifications_token ON pending_verifications(verification_token)",
        )
        .run();
    })();
