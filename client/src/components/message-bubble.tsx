...
export function MessageBubble({
  ...
  // State for media viewer modals
  const [isImageViewerOpen, setIsImageViewerOpen] = useState(false);
  const [isVideoViewerOpen, setIsVideoViewerOpen] = useState(false);
  
  // If we only have an image/video and no text content, handle it differently
  const isImageOnlyMessage = imageData && !content;
  const isVideoOnlyMessage = videoData && !content;
  const isMediaOnlyMessage = (imageData || videoData) && !content;
  // Format time string
