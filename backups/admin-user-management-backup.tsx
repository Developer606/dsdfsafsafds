...
export default function AdminUserManagement() {
  ...
  const { data: plans = [] as SubscriptionPlanWithFeatures[] } = useQuery<
  ...
  const getUniqueSubscriptions = (): string[] => {
    const subscriptions = users
      ...
      .filter(
        (sub: string, index: number, self: string[]) =>
          self.indexOf(sub) === index,
      );
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div className="flex items-center gap-2">
            <motion.h1 
              className="text-3xl font-bold bg-gradient-to-r from-green-500 to-emerald-700 bg-clip-text text-transparent"
      ...
      <motion.div 
        className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border p-6 mb-6"
        variants={itemVariants}
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {plans
                  .filter(
                    (plan) =>
                      !["free", "basic", "premium", "pro"].includes(plan.id),
                  )
                  .map((plan: SubscriptionPlanWithFeatures) => (
                    <DropdownMenuItem
                      key={plan.id}
                      onClick={() =>
                        bulkUpdateSubscription.mutate({
            ...
            <Table>
              ...
              <TableBody>
                {currentUsers.map((user: User, index) => (
                  <TableRow
                    key={user.id}
                    className="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors animate-fadeIn"
                    style={{
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-[200px]">
                          ...
                          {plans
                            .filter(
                              (plan) =>
                                !["free", "basic", "premium", "pro"].includes(
                                  plan.id,
                                ),
                            )
                            .map((plan: SubscriptionPlanWithFeatures) => (
